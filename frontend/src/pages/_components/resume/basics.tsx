...
export default function Basics({
	...
	const renderIcon = (iconName: string, className?: string) => {
		...
		return (
			<IconComponent className={`flex-shrink-0 text-gray-500 ${className}`} />
		);
	};
			...
			<div className="flex flex-col items-start justify-center gap-4 h-fit">
				<div className="flex flex-col sm:flex-row items-center sm:items-start gap-6">
					<div className="text-center sm:text-left flex flex-col gap-2">
						...
						{resume.basics.headline && (
							<p className="mt-1 text-1xl text-primary">
								{resume.basics.headline}
							</p>
						)}
					</div>
				...
				<section className="flex flex-col items-start gap-4">
					<section className="flex flex-row flex-wrap items-start sm:items-center gap-4">
						...
						{resume.basics.phone && (
							<Button variant="outline" className="w-11 h-11" asChild>
								<a href={`tel:${resume.basics.phone}`}>{renderIcon("phone")}</a>
							</Button>
						)}
						...
						{resume.basics.location && (
							<Button variant="outline" className="w-11 h-11" asChild>
								<a
									href={`https://www.google.com/maps/search/?api=1&query=${resume.basics.location}`}
									target="_blank"
						...
						{resume.basics.url &&
							typeof resume.basics.url === "object" &&
					...
					<section className="flex flex-wrap gap-2">
						{resume.basics.customFields?.map(
							(field: (typeof resume.basics.customFields)[0]) => (
								<Badge
									key={field.id}
