...
function EducationRow({ educationItem }: { educationItem: EducationItemType }) {
	return (
		<div className="flex flex-col gap-1 w-full">
			...
			<div className="flex flex-col items-start justify-center mt-2 w-full">
				<p className="text-gray-500 font-mono text-sm whitespace-pre-line">
					{stripHtml(educationItem.summary)}
				</p>
			</div>
		</div>
	);
...
export default function Education({
	...
	if (resume.sections.education.items.length === 0) return null;
		...
		<>
			...
			<div className="flex flex-col items-start justify-between gap-8 h-fit w-full rounded-lg">
				...
				<div className="flex flex-col items-start justify-center gap-6 h-fit w-full">
					{resume.sections.education.items.map(
